...
void Timer1_PWM_Init(u16 arr, u16 psc) {
    ...
    GPIO_InitStructure.GPIO_Pin =
        GPIO_Pin_8 | GPIO_Pin_11;  // TIM1_CH1-->PA8,TIM1_CH4-->PA11
    ...
    GPIO_InitStructure.GPIO_Pin =
        GPIO_Pin_11 | GPIO_Pin_13;  // TIM1_CH2-->PE11,TIM1_CH3-->PE13
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;        // 复用功能
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;      // 推挽复用输出
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;        // 上拉
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;  // 速度100MHz
    ...
    TIM_OCInitStruct.TIM_OCMode =
        TIM_OCMode_PWM2;  // 选择定时器模式(初始化输出比较,TIMx_CNT<TIM_CCRx的时候为有效电平)
    TIM_OCInitStruct.TIM_OCPolarity =
		TIM_OCPolarity_High;  // 输出极性:TIM输出比较极性高(有效电平为高)
	...
	TIM_OCInitStruct.TIM_OCMode =
		TIM_OCMode_PWM1;  // 选择定时器模式(初始化输出比较,TIMx_CNT<TIM_CCRx的时候为有效电平)
    TIM_OCInitStruct.TIM_OCPolarity =
		TIM_OCPolarity_High;  // 输出极性:TIM输出比较极性高(有效电平为高)
    TIM_OCInitStruct.TIM_OutputState = TIM_OutputState_Enable;  // 比较输出使能
...
void Timer8_PWM_Init(u16 arr, u16 psc) {
    ...
    GPIO_InitStructure.GPIO_Pin =
        ...
        GPIO_Pin_9;  // TIM8_CH1-->PC6,TIM8_CH2-->PC7,TIM8_CH3-->PC8,TIM8_CH4-->PC9
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;        // 复用功能
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;      // 推挽复用输出
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;        // 上拉
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;  // 速度100MHz
